...
export function Hero() {
  ...
  // Fine-tuned luxury letterboxing and card morphing as user scrolls
  const scale = useTransform(scrollY, [0, 800], [1, 0.94]);
  const borderRadius = useTransform(scrollY, [0, 800], ["0px", "40px"]);
  const yOffset = useTransform(scrollY, [0, 800], [0, 50]);

  return (
    <section className="relative w-full h-[100dvh] bg-black overflow-visible z-10">
      <motion.div
        style={isMobile ? {} : { scale, borderRadius, y: yOffset }}
        className="relative w-full h-full overflow-hidden bg-black origin-top transform-gpu shadow-[0_30px_100px_rgba(0,0,0,0.8)] border border-white/[0.02]"
      >
